...
- WorkBuddy native plugin `0.9.19-workbuddy.3`: propagate stream errors through the host error channel, reject empty completion streams, and retain upstream model display names with Credits multipliers. Multipliers are informational, not per-request billing formulas; model IDs remain stable.

...
func TestInternationalModelFallback(t *testing.T) {
	...
	check := func(models []pluginapi.ModelInfo) {
		...
		for i, id := range want {
			...
			if decoded["model_config"].(map[string]any)["key"] != id {
				t.Fatalf("upstream model key changed: %s", id)
			}
		}
	}
	...
	if len(cn) != 11 || cn[2].ID != "qfmodel" {
		t.Fatalf("CN fallback must retain upstream Flash addition: %+v", cn)
	}
}
...
func TestLiveQoderModelDiscovery(t *testing.T) {
	...
	if dir == "" {
		t.Skip("live read-only probe not requested")
	}
	paths, err := filepath.Glob(filepath.Join(dir, "qoder-*.json"))
	if err != nil || len(paths) == 0 {
		t.Fatal("No Qoder auth file available for probe")
	}
	data, err := os.ReadFile(paths[0])
	if err != nil {
		t.Fatal("Cannot read probe auth file")
	}
	account, err := parseStored(data)
	if err != nil {
		t.Fatal("Cannot parse probe auth storage")
	}
	models, err := callModelsAPI(account)
	if err != nil {
		t.Logf("Upstream model discovery: %v; account region: %s", err, authRegion(account))
		return
	}
